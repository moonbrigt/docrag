...
./.venv/bin/python -m app.evaluation.real_full_runner                     # 五变体全跑（约 40 分钟，CPU）
...
# 生产参数校准（hybrid_rerank_prod_llm 读 RAG_RERANK_* 配置；env 覆盖做隔离实验，见 BENCHMARK_CARD §12.5）
RAG_RERANK_CANDIDATES=15 RAG_RERANK_MAX_TOKENS=256 ./.venv/bin/python -m app.evaluation.real_full_runner --variants hybrid_rerank_prod_llm --out real_full_p15_t256.json
```

产物 `work/real_full_report.json`（含 `real_full_{bm25,hybrid,rerank,lexical,prod_rerank,p15_t256,p15_t384}.json` 单变体/分组/校准复跑）。检索类指标确定性可复现；answer 类指标受 LLM 采样影响（±0.07 内波动）。预期值与边界见 `docs/BENCHMARK_CARD.md` §12。
