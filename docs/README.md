...
| 层 | 文档 | 内容 | 何时更新 |
|----|------|------|----------|
| 产品 | 顶层 `PRD-文档RAG应用.md` | 产品目标、差异化、用户画像、F1–F10 需求 | 需求变更时 |
...
| 评测协议 | `docs/EVALUATION_PROTOCOL.md` | 数据集生命周期、gold 隔离硬规则、判分协议、容器执行 | 评测流程变化时 |
| 复现指南 | `docs/REPRODUCE.md` | 宿主/容器复现路径、预期输出（expected vs observed） | 命令或产物变化时 |
| 验证记录 | `docs/VALIDATION.md` | 宿主验证与容器验收实测数据、已知问题 | 每次验证后 |

## 2. 关键事实速查（与源码核对过的当前值，2026-08-12 复核）
...
| 解析 | docling 2.117（DocumentConverter + HybridChunker，透传 `page_no` + `bbox`） |
| 嵌入 | BAAI/bge-m3（FlagEmbedding，dense 1024 维 + sparse） |
...
| 融合 | RRF(k=60) → top-20 → 重排 → top-5 |
| LLM | openai SDK + base_url 双后端：Ollama（默认）/ OpenAI 兼容；env 切换 |
...
| API 前缀 | `/api/v1`（19 个端点，见 §4） |
| DB 表 | documents（含 ACL/生命周期列）/ chunks / chunk_fts(FTS5 trigram) / evaluations / document_events（状态流转审计）/ trace（query 原文不落库）/ feedback（单文件 `app.db`，WAL） |
...
| 身份与 ACL | `RAG_TRUSTED_PROXY=true` 时从 X-Rag-Tenant/User/Group 头解析身份（nginx 注入，浏览器不可伪造）；否则本地默认（default 租户）；可见性 fail-closed（无权限 404/空结果），管理操作限属主/管理员 |
...
| MOCK 开关 | `RAG_LLM_MOCK` / `RAG_EMBED_MOCK` / `RAG_RERANK_MOCK` / `RAG_PARSE_MOCK`（compose 默认全 true，离线可跑）；`RAG_TRUSTED_PROXY`（默认 false，true 时信任反向代理注入的 X-Rag-Tenant/User/Group 身份头） |
| 部署 | 单 `docker compose up --build` → http://localhost:3002（nginx 托管前端 + 反代 API；3000 常被占用故用 3002）；隔离验收栈 `docrag-acceptance`（`docker-compose.acceptance.yml`：127.0.0.1:3302、`draccept_data/draccept_models` 卷 + `./work:/work` 评测目录、默认 MOCK + trusted-proxy，见 `docs/REPRODUCE.md`） |
...
- 顶层 `web/`、`phase1-tech-research.md` 为 Superseded 历史稿，仅作追溯，不再作为事实来源。
...
│   │   ├── core/              # parser / embeddings / faiss_store / reranker / llm / metrics / logging / errors
│   │   ├── repositories/      # document_repo / chunk_repo（数据访问）
│   │   ├── services/          # pipeline / document / index / retrieve / rerank / generate / citation
│   │   ├── routes/            # documents / chat / search / meta / evaluation（全部 /api/v1）
│   │   └── evaluation/        # dataset.json（22 条）+ runner（指标计算）
│   ├── requirements*.txt      # 轻量运行时 / 真实模型 / 开发测试 三档依赖
│   └── tests/                 # smoke（health/documents）+ search_eval
├── frontend/
│   ├── src/
│   │   ├── pages/             # Home / Documents / Chat / Evaluation（Spec §7 页面清单）
