...
- **身份**：后端仅在 `RAG_TRUSTED_PROXY=true` 时信任反向代理注入的 `X-Rag-Tenant / X-Rag-User / X-Rag-Group` 头；CORS 不允许浏览器直接发送这些头，nginx `proxy_set_header` 会覆盖客户端同名头。**请勿把后端 8000 端口直接暴露到不可信网络**——绕过可信代理等于绕过身份注入。
